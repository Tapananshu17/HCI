  ...
  const handleSave = () => {
    ...
    if (formData.newPassword) {
      ...
      if (userProfile.password) {
        // Require current password to be entered and match
        ...
        if (formData.currentPassword !== userProfile.password) {
          const errorMsg = userProfile.language === 'en' ? 'Current password is incorrect' :
                          userProfile.language === 'hi' ? 'वर्तमान पासवर्ड गलत है' :
                          userProfile.language === 'te' ? 'ప్రస్తుత పాస్‌వర్డ్ తప్పు' :
                          userProfile.language === 'ta' ? 'தற்போதைய கடவுச்சொல் தவறானது' :
                          userProfile.language === 'bn' ? 'বর্তমান পাসওয়ার্ড ভুল' :
    ...
    const updatedProfile = {
      ...
      password: formData.newPassword || userProfile.password
    };
    
    setUserProfile(updatedProfile);
    
    // Save to localStorage
    localStorage.setItem('pathfinder_user_name', formData.name);
    localStorage.setItem('pathfinder_user_grade', formData.grade);
    if (formData.age) {
      localStorage.setItem('pathfinder_user_age', formData.age);
    }
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="pt-6 border-t">
              ...
              <div className="space-y-4">
                {/* Current Password - shown only if user has a password set */}
                ...
                <div>
                  <Label htmlFor="newPassword">
                    {userProfile.password ? t.newPassword : t.password} 
                  </Label>
                  <div className="flex items-center gap-2 mt-2">
                    <Lock className="h-4 w-4 text-gray-400" />
                    <div className="flex-1 relative">
